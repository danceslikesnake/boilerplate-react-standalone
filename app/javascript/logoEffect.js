...
function initParticles() {
  particles = [];
  const numberOfParticles = (w * h) / 700;
...
function calculateField() {
  ...
  for (let x = 0; x < columns; x++) {
    for (let y = 0; y < rows; y++) {
      const color = buffer32[y * size * w + x * size];
      if (color) {
...
function drawText(callback) {
  ...
  logo.src =
    sw > 768
      ? require('../images/home-logo-500.png')
      : require('../images/home-logo.png');
  logo.onload = () => {
    const leftMargin = (w - logo.width) / 2;
    const topMargin = (h - logo.height) / 2;
...
export default function startLogoNoise() {
  setup(draw);
}
